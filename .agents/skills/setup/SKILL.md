...
2. 环境变量替换提示与确认（Env Replacement Prompt & Confirm）
3. 预检（Preflight）
4. 对齐补齐（Reconcile）
5. 验证（Verify）
6. 运行信息提示（Runtime Info Prompt）
...
> 注意：此阶段只允许输出“项目名输入提示”，不得提前输出环境变量提示或运行信息提示。
...
- 当前仅完成模板占位符替换，不代表可直接用于生产。
- 需要将前后端 `.env.example` 中的敏感配置替换为真实环境变量（如数据库密码、JWT 密钥、第三方密钥等）。
- 在用户确认或知晓后，才继续后续阶段。
...
- `GET /health` 可访问
- 前端首页可访问，且 PC 端布局可正常展示
- 移动端首页可访问，且响应式布局可用

---

## 阶段 D：运行信息提示（Runtime Info Prompt）

完成验证后，必须单独输出运行信息，至少包含：

- Host/IP（例如 `localhost` / `127.0.0.1`）
- 前端访问地址与端口
- 后端访问地址与端口
- 健康检查地址
- Metrics 地址（若有）

> 注意：运行信息提示必须在验证完成后输出，不得和“项目名输入提示”或“环境变量替换提示”合并在同一步骤。
...
7. 实际执行的验证命令
8. 最高优先级风险（Top Risk）
9. 环境变量配置提示（前后端哪些变量必须改为真实值）
10. 初始化后运行信息（运行方式、协议、主机/IP、端口、关键访问 URL）
11. 前端首页验证结果（PC 端与移动端是否均可用）
